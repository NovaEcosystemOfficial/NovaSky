...
function renderPanelIdle(panelBody) {
  panelBody.innerHTML = `
...
function renderPanelTarget(panelBody, cardRoot, missionCount, showBriefing = false) {
 ...
 * @param {{ panelBody: HTMLElement, appEl: HTMLElement }} ctx
 */
export function installPanelBridge(app, ctx) {
  const { panelBody, appEl } = ctx;
  ...
  const syncPanel = () => {
    ...
    if (isOpen) {
      ...
      renderPanelTarget(panelBody, cardRoot, count, showBriefing);
    } else {
      showBriefing = false;
      setStatus("Pronto");
      renderPanelIdle(panelBody);
